/**
 * SpeakerMissionPanel - 발언자(파트너)용 미션 관리 패널
 *
 * 호스트가 아닌 발언자(파트너)가 자신에게 온 미션을 관리하는 바텀시트
 * - 자신에게 온 미션만 표시
 * - 수락/거절/성공/실패 처리 가능
 */
...
interface SpeakerMissionPanelProps {
  roomId: string
  /** 현재 발언자의 파트너 ID */
  myPartnerId: string
  isOpen: boolean
...
export function SpeakerMissionPanel({
  roomId,
  myPartnerId,
  isOpen,
  onClose,
}: SpeakerMissionPanelProps) {
  ...
  const {
    ...
    enabled: isOpen,
    typeFilter: ['mission'],
    recipientPartnerId: myPartnerId,
    enableRealtime: true,
  })
