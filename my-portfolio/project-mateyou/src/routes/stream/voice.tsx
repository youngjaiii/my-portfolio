/**
 * 보이스 스트림 목록 페이지
 * 
 * 기능:
 * - 보이스(오디오) 스트림 목록 조회
 * - 정렬 (최신순/시청자순/시작순)
 * - 방 생성 버튼 (모달 열기)
 */

import { StreamCard, StreamCardSkeleton, StreamEmptyState } from '@/components/features/stream/StreamCard'
import { StreamFAB } from '@/components/features/stream/StreamFAB'
import { CreateStreamSheet } from '@/components/modals/CreateStreamSheet'
import { useVoiceRoomConnection } from '@/contexts/VoiceRoomProvider'
import { useAuth } from '@/hooks/useAuth'
import { useStreamRooms } from '@/hooks/useStreamRooms'
import { useVoiceRoom } from '@/hooks/useVoiceRoom'
import { createFileRoute, useNavigate } from '@tanstack/react-router'
import { Radio } from 'lucide-react'
import { useState } from 'react'

export const Route = createFileRoute('/stream/voice')({
  component: RouteComponent,
})

type SortOption = 'latest' | 'viewers' | 'starting'

const sortOptions: { value: SortOption; label: string }[] = [
  { value: 'latest', label: '최신순' },
  { value: 'viewers', label: '시청자순' },
  { value: 'starting', label: '시작순' },
]

function RouteComponent() {
  const navigate = useNavigate()
  const { user, isAuthenticated } = useAuth()
  const [sortBy, setSortBy] = useState<SortOption>('latest')
  const [showCreateModal, setShowCreateModal] = useState(false)
  const [initialStreamType, setInitialStreamType] = useState<'audio' | 'video'>('audio')
  const [isMobileLiveMode, setIsMobileLiveMode] = useState(false)

  // FAB에서 방송 시작 시트 열기
  const handleOpenCreateSheet = (streamType: 'audio' | 'video' = 'audio', mobileMode = false) => {
    setInitialStreamType(streamType)
    setIsMobileLiveMode(mobileMode)
    setShowCreateModal(true)
  }

  // 현재 연결된 보이스룸 확인 (호스트 중복 방지용)
  const { currentRoomId, isConnected } = useVoiceRoomConnection()
  const { isHost: isCurrentRoomHost } = useVoiceRoom(currentRoomId || undefined)
  
  // 호스트가 진행중인 방이 있는지 확인
  const hasActiveHostingRoom = isConnected && currentRoomId && isCurrentRoomHost

  // 스트림 데이터 가져오기
  const { data: streams = [], isLoading, error } = useStreamRooms({ 
    streamType: 'audio',
    status: 'live',
    limit: 100 
  })

  // 정렬 적용
  const sortedStreams = [...streams].sort((a, b) => {
    switch (sortBy) {
      case 'viewers':
        return b.viewerCount - a.viewerCount
      case 'starting':
        return new Date(a.whenStart).getTime() - new Date(b.whenStart).getTime()
      case 'latest':
      default:
        return new Date(b.whenStart).getTime() - new Date(a.whenStart).getTime()
    }
  })

  // 방 생성 버튼 클릭
  const handleCreateClick = () => {
    if (!isAuthenticated) {
      alert('로그인이 필요합니다')
      return
    }
    setShowCreateModal(true)
  }

  return (
    <div 
      className="min-h-screen bg-white"
      style={{
        paddingTop: 'calc(env(safe-area-inset-top, 0px) + 56px)',
        paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 80px)',
      }}
    >
      <div className="px-4">
        {/* 필터 및 정렬 */}
        <div className="flex items-center justify-between py-3 border-b border-gray-100">
          <span className="text-sm text-gray-500">
            {isLoading ? '로딩 중...' : `총 ${sortedStreams.length}개`}
          </span>
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as SortOption)}
            className="text-sm text-gray-600 bg-transparent border-none outline-none cursor-pointer"
          >
            {sortOptions.map((opt) => (
              <option key={opt.value} value={opt.value}>
                {opt.label}
              </option>
            ))}
          </select>
        </div>

        {/* 에러 상태 */}
        {error && (
          <div className="mt-4 p-3 bg-red-50 text-red-600 text-sm rounded-lg">
            스트림을 불러오는데 문제가 발생했습니다.
          </div>
        )}

        {/* 스트림 그리드 */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mt-4">
          {isLoading ? (
            Array.from({ length: 8 }).map((_, idx) => <StreamCardSkeleton key={`skeleton-${idx}`} />)
          ) : sortedStreams.length > 0 ? (
            sortedStreams.map((stream) => (
              <StreamCard
                key={stream.id}
                stream={stream}
                onClick={() => {
                  // 호스트가 진행중인 방이 있고, 다른 방에 입장하려고 하면 경고
                  if (hasActiveHostingRoom && stream.id !== currentRoomId) {
                    alert('이미 진행 중인 방송이 있습니다.\n방송을 종료한 후 다른 방에 입장할 수 있습니다.')
                    return
                  }
                  navigate({ to: '/stream/chat/$roomId', params: { roomId: stream.id } })
                }}
              />
            ))
          ) : (
            <div className="col-span-full">
              <StreamEmptyState
                message="현재 진행 중인 보이스 방송이 없습니다"
                icon={<Radio className="w-12 h-12 mb-3 opacity-50 text-purple-300" />}
              />
            </div>
          )}
        </div>
      </div>

      {/* 방송 시작 FAB */}
      <StreamFAB onOpenCreateSheet={handleOpenCreateSheet} />

      {/* 방 생성 시트 */}
      <CreateStreamSheet
        isOpen={showCreateModal}
        onClose={() => setShowCreateModal(false)}
        initialStreamType={initialStreamType}
        isMobileLiveMode={isMobileLiveMode}
      />
    </div>
  )
}
