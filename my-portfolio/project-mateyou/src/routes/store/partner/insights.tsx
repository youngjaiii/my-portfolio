...
import { RefreshCw, TrendingUp, Package, ShoppingBag, Truck, RotateCcw, Calendar, X, Search } from 'lucide-react';
...
export const Route = createFileRoute('/store/partner/insights')({
  component: PartnerInsightsPage,
});
...
interface StatsData {
  ...
  period: string;
  filter: { start_date: string; end_date: string; product_id: string | null };
  summary: {
    total_orders: number;
...
function PartnerInsightsPage() {
  ...
  const fetchStats = useCallback(async () => {
    setIsLoading(true);
    try {
      const params: { period?: string; start_date?: string; end_date?: string } = { period };
      ...
      const response = await storeOrdersApi.getPartnerStats(params);
  ...
  const chartData = useMemo<ChartDataPoint[]>(() => {
    ...
    return allPeriodLabels.map(label => {
      ...
      ['pending', 'shipped', 'delivered'].forEach(status => {
        ...
        if (shipment?.by_period?.[label]) {
          shipmentsCount += shipment.by_period[label].count || 0;
        }
      });
      ...
      ['pending', 'approved', 'rejected', 'completed'].forEach(status => {
        ...
        if (refund?.by_period?.[label]) {
          refundsCount += refund.by_period[label].count || 0;
        }
      });
  ...
  const shipmentPending = stats?.shipments?.pending?.count || 0;
  const shipmentShipped = stats?.shipments?.shipped?.count || 0;
  const shipmentDelivered = stats?.shipments?.delivered?.count || 0;
  const refundPending = stats?.refunds?.pending?.count || 0;
  const refundCompleted = stats?.refunds?.completed?.count || 0;
  const refundRejected = stats?.refunds?.rejected?.count || 0;
